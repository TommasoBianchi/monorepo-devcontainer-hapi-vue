...
const loadSchwiftyPluginsOptions = () => {
  const plugins = ["lib", ...fs.readdirSync("./").filter((dir) => dir.endsWith("-plugin"))];
  const pluginsWithMigrations = plugins.filter(
    (plugin) => fs.existsSync(`${plugin}/plugins/@hapipal.schwifty.js`) && fs.existsSync(`${plugin}/migrations`)
  );
...
module.exports = Hoek.applyToDefaults(
  {
    migrations: {
      directory: loadSchwiftyPluginsOptions().map(({ migrationsDir = "migrations " }) =>
